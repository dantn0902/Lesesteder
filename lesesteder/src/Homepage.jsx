...
export default function Homepage() {
  ...
  const handleFilterChange = (key, value) => {
    ...
    if (value === undefined) {
      ...
      // It’s a slider update
      setLibraries(() =>
        librariesData.filter((library) => library.komfor[key] >= value)
      );
    }
  };
  ...
  useEffect(() => {
    ...
    const filteredLibraries = librariesData.filter(
      (library) =>
        filtersOn.every(
          (filter) =>
            library.fasiliteter[filter] ||
            library.komfor[filter] ||
            library.tjenester[filter]
        ) // make sure library matches all filters
    );
  ...
  return (
    <div className="content">
      ...
      <div className={`filterScreen ${openFilterScreen ? "active" : ""}`}>
        <FilterScreen
          onClose={() => handleFilterScreen()}
          fasiliteter={f}
          tjenester={t}
          komfor={k}
          onOptionToggle={handleFilterChange}
        />
      </div>
      <div className="cards">
        {libraries.map((bib) => (
